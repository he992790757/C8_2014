			...
			case X0 : if(System.Dsp.CheckMode.Zoom != X0 )
					  {
						...
						OSTimeDly(10); //DSP_SENDB(0xe2,NULL," "); 
					   } break ; 
			
			case X1 :  if(System.Dsp.CheckMode.Zoom != X1 )
					  {
						//	Scen_Send(0x2C,0x30);  	
						Dsp_Send_Data(0x2C,0x20,0,0,D_NON); 	
...
void PhotographOne()
{
	...
	//	Dsp_Send_Data(0x21,0x00,0x01,0x01,D_NON);
		...
		switch(System.Led.Statu)
		{
			case DB_UV_EXPOSURE1_ON : if(System.Dsp.Sensor == SENSOR1){ Scen_Send(0xE7, G_SENCE_DB_UV);  OSTimeDly(100);  Double_UV_Exposure1_On();}
										else{Select_Send(0x21); OSTimeDly(100);} break; 
			case DB_UV_EXPOSURE2_ON : if(System.Dsp.Sensor == SENSOR1){  Scen_Send(0xE7, G_SENCE_DB_UV2);  OSTimeDly(100);  Double_UV_Exposure2_On(); }
										else{Select_Send(0x21); OSTimeDly(100);} break; 
			case BOT_W_ON 			: if(System.Dsp.Sensor == SENSOR1){  Scen_Send(0xE7, G_SENCE_B_W);  OSTimeDly(100);   Bot_W_On(); }	   //4
										else{Select_Send(0x21); OSTimeDly(100);} break; 
			default :  Select_Send(0x21); OSTimeDly(300); 	break ; 
		}
	 	OSTimeDly(20);	
